...
from gb_eli_mcp.coverage import build_coverage
from gb_eli_mcp.server import gb_coverage


def _coverage():
    fn = getattr(gb_coverage, "fn", gb_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("GB_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("gb_coverage" in line for line in lines), (
        "an audit entry was written but it does not name gb_coverage"
    )
