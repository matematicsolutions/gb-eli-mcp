...
SOURCE = 'legislation.gov.uk, caselaw.nationalarchives.gov.uk and gov.uk'
...
_FAMILIES: list[dict] = [{'name': 'Legislation', 'tool': 'gb_search / gb_get_act / gb_get_text / gb_recent_legislation'}, {'name': 'Case law (Find Case Law)', 'tool': 'gb_search_case_law / gb_get_case'}, {'name': 'Government guidance (gov.uk)', 'tool': 'gb_search_govuk / gb_get_govuk_content'}]

_GAPS: list[dict] = [{'id': 'GB-001', 'family': 'Legislation', 'missing': "legislation.gov.uk has no native /eli/ namespace; eli_uri carries the UK's own persistent identifier URI instead.", 'fallback': 'Do not expect a data.europa.eu-style ELI path.'}, {'id': 'GB-002', 'family': 'Legislation', 'missing': 'Revised (as amended) text reflects the effective date shown in restrict_start_date and may lag behind the latest amendments.', 'fallback': 'Check restrict_start_date on the record and the changes-to-legislation banner at source.'}, {'id': 'GB-003', 'family': 'Case law (Find Case Law)', 'missing': 'Find Case Law is a separate service with no shared identifier scheme with legislation.gov.uk, and pre-April-2025 documents have a different URI derivation.', 'fallback': 'Resolve judgments through the Find Case Law service itself.'}]
