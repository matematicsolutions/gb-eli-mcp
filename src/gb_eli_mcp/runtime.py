...
Pulls a tiny static ``gb-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/gb-runtime.json
...
Opt-out: set ``GB_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "GB_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/gb-eli-mcp/releases/latest/download/gb-runtime.json.gz",
)
_CACHE_NAME = "gb-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "gb-eli-mcp-runtime (+https://github.com/matematicsolutions/gb-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("GB_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "gb-eli"
